...
<%
    ...
    if (email == null) {
        ...
        try {
            ...
            ResultSet rs1 = st.executeQuery("select * from like1 where vcode='" + vcode + "' AND ucode='" + ucode + "'");
            if (rs1.next()) {
                ...
                if (vcode == "") {
                    ...
                    ResultSet rs2 = st1.executeQuery("select * from unlike where vcode='" + vcode + "' AND ucode='" + ucode + "'");
                    if (rs2.next()) {
                        PreparedStatement ps1 = cn.prepareStatement("delete from unlike where vcode='" + vcode + "' AND ucode='" + ucode + "'");
                        if (ps1.executeUpdate() > 0) { }
                    }
                    PreparedStatement ps = cn.prepareStatement("insert into like1 values(?,?)");
